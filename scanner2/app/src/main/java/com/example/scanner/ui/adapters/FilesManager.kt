...
class FilesManager() {
    ...
    fun getPdfFiles(context: Context): MutableList<PdfFile> {
        ...
        context.contentResolver.query(allFilesUri, projection, selection, selectionArgs, sortOrder)
            ?.use { cursor ->
                ...
                while (cursor.moveToNext()) {
                    ...
                    val textSize=if(size<1024*1024){
                        val kb=size/1024.0
                    ...
                    //переводим секунды получившиеся в миллисекунды
                    val formatDate=formatDate(date*1000)
                    ...
                    pdfList.add(PdfFile(contentUri, name, textSize, formatDate, id))
                }
            }
        return pdfList
    }
